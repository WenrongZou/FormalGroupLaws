...
theorem comm_iff_coeff_symm :
  ...
  · intro h; ext n
    ...
    · simp [subst_symm]
      have aux : (coeff R n) (X₁ ^ n 1 * X₀ ^ n 0) = 1 := by
        simp [X_pow_eq, monomial_mul_monomial]
    ...
    · intro d hd; simp [subst_symm]
      have aux : (coeff R n) (X₁ ^ d 0 * X₀ ^ d 1) = 0 := by
        simp [X_pow_eq, monomial_mul_monomial]
        rw [coeff_monomial, if_neg]
...
theorem comm_iff_coeff_symm' :
  F.comm ↔ ∀ (i j : ℕ), coeff _ (coeff_two i j) F.toFun - coeff _ (coeff_two j i) F.toFun = 0 := by
  constructor
  · intro hF
  ...
  · intro h
    ...
    intro i j
    conv => rhs; rw [←add_zero ((coeff R (coeff_two j i)) F.toFun), ←h i j]
    ring
...
theorem comm_iff_no_nonzero_torsion_nilpotent :
  ...
  · intro hr
    ...
    have mem_ideal : ∀ (i j : ℕ), ∀ (I : Ideal R), I.IsPrime →
      (coeff _ (coeff_two i j) F.toFun - coeff _ (coeff_two j i) F.toFun) ∈ I := by
      intro i j I hI
    ...
    have mem_nilpotent : ∀ (i j : ℕ),
      IsNilpotent (coeff _ (coeff_two i j) F.toFun - coeff _ (coeff_two j i) F.toFun) :=
      fun i j => nilpotent_iff_mem_prime.mpr (mem_ideal i j)
    have mem_torsion : ∀ (i j : ℕ),
      addOrderOf (coeff _ (coeff_two i j) F.toFun - coeff _ (coeff_two j i) F.toFun) ≠ 0 := by
      intro i j

      sorry
    have mem_zero : ∀ (i j : ℕ),
      (coeff _ (coeff_two i j) F.toFun - coeff _ (coeff_two j i) F.toFun) = 0 :=
